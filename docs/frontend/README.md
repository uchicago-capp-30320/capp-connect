...
## Outstanding Issues
While the basic layout and app works there are some unresolved issues with using the endpoints. Due to this, it is not currently possible to create posts in the app and have them posted to the database, it is not possible to edit user profile details, and while Search works for the posts and resources, it doesn't currently work for the directory. There is currently code in the frontend to do all of these things so these issues could likely be resolved without much more work.
